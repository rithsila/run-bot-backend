...
const jsonString = (label: string) =>
  Joi.string().custom((v, helpers) => {
    ...
    try {
      ...
      return helpers.error('any.invalid', { message: `${label} must be valid JSON` });
    }
...
export const envValidationSchema = Joi.object({
  ...
  JWT_ACCESS_PUBLIC_KEY_BASE64: b64Pem('JWT_ACCESS_PUBLIC_KEY_BASE64')
    .when('JWT_ACCESS_ALG', { is: 'HS256', then: Joi.string().allow('').default(''), otherwise: Joi.string().required() }),
  JWT_ACCESS_SECRET: Joi.string().when('JWT_ACCESS_ALG', { is: 'HS256', then: Joi.string().min(16).required(), otherwise: Joi.string().allow('').default('') }),
