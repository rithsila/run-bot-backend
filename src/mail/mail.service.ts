...
export class MailService {
    ...
    private async init() {
        ...
        if (!user || !pass) {
            this.logger.warn(
                'MAIL_FROM_EMAIL / GMAIL_APP_PASSWORD not set — Mail disabled.',
            );
            return;
        }
        ...
        try {
            ...
            const msg = `SMTP verify failed — mail disabled. ${e?.message ?? e}`;
            if (isProd) {
    ...
    private ensureTransport() {
        if (!this.transporter) {
            throw new Error(
                'Mail transport unavailable (bad creds or init failed).',
            );
        }
    ...
    async send(opts: {
        ...
        const text =
            opts.text ?? (opts.html ? this.stripHtml(opts.html) : undefined);
